...
function getConfirmationForDish(confirmations, dishId, requestedQuantity) {
  ...
  if (confirmation.status === 'partial') {
    return {
      ...confirmation,
      available_quantity: normalizeQuantity(confirmation.available_quantity),
    };
  }
...
function buildMenuSummary({ dishes = [], entries = [], confirmations = [] }) {
  ...
  const items = [...entriesByDish.entries()].map(([dishId, dishEntries]) => {
    ...
    Object.entries(allocation.by_user).forEach(([userId, quantity]) => {
      addUserTotal(final.by_user, userId, quantity, price);
    });
    ...
    return {
      dish: {
        ...
        name: dish.name,
        price,
        image_file_id: dish.image_file_id || '',
      },
